...
async function getIcon(req, env, url) {
  ...
  /*
   * 実際の公開画像は root.js / safety.js が先に処理する。
   * ここは旧互換経路として残す。
   */
  h.set(
    'cache-control',
...
async function putIcon(req, env, dev) {
  ...
  await env.DB
    .prepare(
      'UPDATE devices SET icon_ver=? WHERE device_id=?'
    )
    .bind(
      ver,
      dev.device_id
...
async function deleteIcon(req, env, dev) {
  ...
  await env.DB
    .prepare(
      'UPDATE devices SET icon_ver=0 WHERE device_id=?'
    )
    .bind(
      dev.device_id
    )
...
// R2削除は失敗しても本処理を止めない
async function tryDeleteIconObject(
  env,
  memberId
) {
  try {
...
function publicMe(dev) {
  ...
  return {
    ...
    goal_weight:
      dev.goal_weight === null ||
      dev.goal_weight === undefined
        ? null
        : Number(
            dev.goal_weight
          ),
...
async function dropLeader(
  ...
  try {
    if (gid) {
      await env.DB
        .prepare(
          'DELETE FROM group_leaders WHERE group_id=? AND member_id=?'
        )
        .bind(
          gid,
          memberId
      ...
      await env.DB
        .prepare(
          'DELETE FROM group_leaders WHERE member_id=?'
        )
        .bind(
          memberId
        )
...
async function dropAllLeaders(
  ...
  try {
    await env.DB
      .prepare(
        'DELETE FROM group_leaders WHERE group_id=?'
      )
      .bind(gid)
      .run();
  } catch (e) {
...
async function requireLedGroup(
  env,
  dev
) {
  if (!dev.group_id) {
...
async function route(
  ...
  /* ==========================================================
     ...
     BAN中でも本人による利用データ削除だけは許可。

     banned判定より先にDELETEを処理する。
     ========================================================== */
  ...
  if (
    ...
    if (
      ...
      const rs =
        await env.DB
          .prepare(
            'SELECT ymd,kg FROM weights WHERE device_id=? ORDER BY ymd ASC'
          )
          .bind(
            dev.device_id
          )
...
async function patchGroup(
  ...
  await env.DB
    .prepare(
      `UPDATE groups SET ${sets.join(',')} WHERE group_id=?`
    )
    .bind(
      ...vals
    )
...
async function addLeader(
  ...
  try {
    await env.DB
      .prepare(`
        INSERT OR IGNORE INTO group_leaders (
          group_id,
          member_id,
          created_at
        )
...
async function addWatching(
  ...
  await env.DB
    .prepare(`
      INSERT OR IGNORE INTO watching (
        device_id,
        group_id,
        created_at
      )
...
async function listRivals(
  ...
  const rs =
    await env.DB
      .prepare(`
        SELECT
          r.rival_member_id AS member_id,
          d.nickname,
          d.icon_ver
        ...
        LEFT JOIN devices d
          ON d.member_id=r.rival_member_id
...
async function listBlocks(
  ...
  const rs =
    await env.DB
      .prepare(`
        SELECT
          b.blocked_member_id AS member_id,
          d.nickname,
          d.icon_ver
        ...
        LEFT JOIN devices d
          ON d.member_id=b.blocked_member_id
